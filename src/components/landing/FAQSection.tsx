...
const faqs = [
  ...
  {
    question: "Pourquoi dois-je signer une autorisation avant le scan ?",
  ...
  {
    question: "Comment fonctionne le coffre-fort de preuves ?",
    answer: "Chaque action (scan, import, modification) est enregistrée dans un journal immuable avec horodatage et empreinte cryptographique. En cas d'audit, vous pouvez exporter un rapport certifié de toutes les preuves.",
  },
  {
          ...
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem 
                key={index} 
                value={`item-${index}`}
                className="glass-card rounded-xl px-6 border-glass-border data-[state=open]:border-primary/30"
              >
